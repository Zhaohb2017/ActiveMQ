...
debug_file_name = "debugfile/"+'%s.debug.log' % (time.strftime("%Y%m%d"))
...
class SQLMaker:
    ...
    # 时间戳转换
    def _time_stamp_conversion(self,data, type='y_m_d'):
        timeStamp = data
    ...
    # 数据结构解析
    def data_structure_analysis(self,message, DB,dbname):

        db = DB
        # -------------------------------------
        db_name = dbname
        apptype = message["apptype"]  #分表ID
        ...
        create_time = self._time_stamp_conversion(date)  # 房间创建时间
        paytimes = message["now_time"]
        new_times = self._time_stamp_conversion(date, type="h_m_s")  # 最新更新时间
        ...
        if len(players) is 0:
            ...
            _winner = self._winner_data(message)
            for w in _winner:
                for k, v in w.items():
                    if k == "mid":
                        winner_ID.append(v)
            ...
            for user_dict in players:
                ...
                create_time = self._time_stamp_conversion(date, )  # 房间创建时间
                paytimes = message["now_time"]
                new_times = self._time_stamp_conversion(paytimes, type="h_m_s")  # 最新更新时间
                ...
                sql = '''
                    INSERT INTO {db_name}.club_user_day{apptype}_{db_sign} (
                        `mid`,
                        `clubid`,
                        `date`,
                    	`paytimes`,
